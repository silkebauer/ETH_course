...
obs_methods <- data.frame(read.csv("e:/Github/ETH_course/ObservationMethods.csv", h=T, sep=";"))
...
knitr::kable(
  head(obs_methods, 20), caption = 'Characteristics of different observation methods. MTR is migration traffic rate (a standardized migration intensity)',
...
track_methods <- data.frame(read.csv("TrackingMethods.csv", h=T, sep=";"))
...
```{r nice-tab, tidy=TRUE, echo=FALSE}
knitr::kable(
  head(track_methods, 20), caption = 'Characteristics of different tracking techniques. Adapted from Robinson et al. (2010)',
  format.args = list(width = 30),
  booktabs = TRUE,
)
```
